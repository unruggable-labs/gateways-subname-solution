...
const ContractDeployment: React.FC<ContractDeploymentProps> = ({
  ...
  const deployContract = async () => {
    ...
    try {
      ...
      console.log("switch", deploymentChainConfig.appkitChain);
      // Switch to the deployment chain
      await switchNetwork(deploymentChainConfig.appkitChain);
      ...
      const explorerUrl = deploymentChainConfig?.etherscan.browserUrl;
  ...
  const verifyContract = async () => {
    ...
    try {
        const verificationData = {
            apikey: deploymentChainConfig.etherscan.apiKey,
            module: 'contract',
        ...
        const submitResponse = await fetch(deploymentChainConfig.etherscan.apiUrl, {
            method: 'POST',
            headers: {
        ...
        if (submitResult.status === '1' && submitResult.result) {
            ...
            const checkVerification = async () => {
                ...
                const checkUrl = new URL(deploymentChainConfig.etherscan.apiUrl);
                checkUrl.searchParams.append('apikey', deploymentChainConfig.etherscan.apiKey);
                ...
                if (checkResult.result === 'Pending in queue') {
                    ...
                    const successMessage = (<>Contract verified successfully! View it on <a href={`${deploymentChainConfig.etherscan.browserUrl}/address/${deployedContract.address}#code`} target="_blank" rel="noopener noreferrer">Etherscan</a>.</>);
